...
Para que a aplicação desse filtro seja feita de forma adequada, deve-se seguir os seguintes passos: Calcular o logaritmo natural da imagem, aplicar a transformada de fourier discreta, multiplicar o espectro com a mascara do filtro no domínio da frequência, aplicar a transformada inversa e, por ultimo, fazer a exponencial da imagem filtrada.

O código completo pode ser obtido [aqui](gabrielsig.github.io/codes/homomorphic_filter.py).

#### 1.2. Explicando o código

Iniciamos o algoritmo definindo os parâmetros que serão usados na construção do filtro e carregando a imagem em tons de cinza. Obtemos o tamanhos ideais para o calculo da DFT e criamos uma cópia da imagem com uma borda preta para que tenhamos uma matriz nesse tamanho ideal. Logo em seguida fazemos o logaritmo da imagem:
...
Aplicamos a DFT no logaritmo da imagem e trocamos os quadrantes do resultado. Dessa forma o processo de filtragem será facilitado:
...
Então criamos a mascara do filtro homomórfico com base no tamanho da DFT e nos parâmetros definidos no inicio do programa. Aplicamos o filtro ao espectro, realizamos a troca dos quadrantes novamente e fazemos a transformada invesa:
...
Por fim realizamos a exponenciação para obtermos a imagem final:
...
Aplicamos o filtro a uma cena escura e com iluminação irregular. Como podemos observar nas imagens abaixo, houve um aumento considerável de detalhamento nas áreas mais escuras, apesar da inevitavel elevação do nível de ruído:

![original](gabrielsig.github.io/images/homomorphic/original.png)
...
Nesse exercício usamos o algoritmo de K means para realizar a clusterização de cores de uma imagem, obtendo as ‘n’ cores mais proeminentes que forma a cena. Usamos a inicialização aleatória de centros e aplicamos o algoritmo algumas vezes na mesma imagem a fim de verificarmos se as cores achadas iriam divergir muito de uma iteração para a outra.

O código completo do programa pode ser encontrado [aqui](gabrielsig.github.io/codes/kmeans_color.py)

#### 3.2. Explicando o código

Começamos criando as variáveis necessárias, carregando a imagem e criando um array de tamanho `(height*width) x 3` que armazena as intensidades das 3 componentes de cor de cada pixel da imagem original:
...
Aplicamos o algoritmo de k-means `n_iterations` vezes com os centros sendo iniciados de forma aleatória. Em cada uma das iterações,  criamos e salvamos uma imagem clusterizada com as cores obtidas pelo algoritmo. Além disso, ordenamos ordenamos o array de centros por matiz e geramos uma nova imagem com a paleta de cores, assim temos uma representação visual mais clara das diferenças entre as iterações. Por fim mostramos as imagens e salvamos os resultados de cada iteração:
...
Abaixo podemos ver os resultados de cada uma das iterações, tal como as paletas de cores e a lista de matizes. Se observarmos as imagens individualmente, não seria claro imediatamente as diferenças entre elas, e vemos na lista de matizes e paletas que as diferenças entre as cores de cada iteração não são tão proeminentes assim. Ao colocarmos as imagens em um loop, as diferenças ficam mais claras.

Essas diferenças podem ser atribuídas a inicialização aleatória de centros, de forma que o algoritmo não converge nos mesmos pontos após o número de rodadas definido. Porém, se aumentarmos o esse parâmetro, o algoritmo tende a convergir nos mesmos pontos, pois a imagem não tem uma variedade de cores muito elevada

![original](gabrielsig.github.io/images/kmeans/original.jpg)

![hues](gabrielsig.github.io/images/kmeans/hues.png)

![palette](gabrielsig.github.io/images/kmeans/palette.png)

![clustered](gabrielsig.github.io/images/kmeans/clustered.gif)
